...
- тарифный слот занимает пользователь со `status: active` и `inTariff: true`;
  отображаемая роль не участвует в подсчёте;
...
6. оставить domain ids (`RoleId`, `UserStatus`, `ModuleId`) независимыми от i18n.
...
| гостевая активация без расходования слота | `?scenario=limit`, включить Дмитрия Лебедева |
| поиск по отображаемой роли | раскрыть поиск и ввести `Аудитор` |
| недоступный по роли модуль | открыть `Аналитика` в default fixture |
